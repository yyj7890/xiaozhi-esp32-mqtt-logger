...
void WifiConfigurationAp::StartWebServer()
{
    ...
    // Local-IoT logging is deliberately configured through its own API and NVS
    // namespace. Neither endpoint returns password or discovery-token values.
    httpd_uri_t aiot_log_config = {
        .uri = "/aiot-log/config",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            const AiotLogConfig config = AiotLogConfigStore::Load();
            cJSON* json = cJSON_CreateObject();
            if (json == nullptr) {
                httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Unable to load configuration");
                return ESP_FAIL;
            }
            ...
            cJSON_AddNumberToObject(lan, "port", config.lan.port);
            cJSON_AddBoolToObject(lan, "tls", false);
            cJSON_AddBoolToObject(lan, "hasPassword", config.lan.has_password);
            cJSON_AddBoolToObject(lan, "hasDiscoveryToken", config.lan.has_discovery_token);
            cJSON* remote = cJSON_AddObjectToObject(json, "remote");
            cJSON_AddStringToObject(remote, "username", config.remote.username.c_str());
            cJSON_AddStringToObject(remote, "host", config.remote.host.c_str());
            cJSON_AddNumberToObject(remote, "port", 8883);
            cJSON_AddBoolToObject(remote, "tls", true);
            cJSON_AddBoolToObject(remote, "hasPassword", config.remote.has_password);
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &aiot_log_config));

    httpd_uri_t aiot_log_submit = {
        .uri = "/aiot-log/submit",
        .method = HTTP_POST,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            if (req->content_len == 0 || req->content_len > 2400) {
            ...
            const int read = httpd_req_recv(req, body.get(), req->content_len);
            if (read <= 0) {
                httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Unable to read request");
                return ESP_FAIL;
            }
            body[read] = '\0';
            cJSON* json = cJSON_Parse(body.get());
            if (json == nullptr) {
                httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Invalid JSON");
                return ESP_FAIL;
            }
            ...
            const cJSON* lan_host = cJSON_GetObjectItemCaseSensitive(lan, "manualHost");
            const cJSON* lan_port = cJSON_GetObjectItemCaseSensitive(lan, "port");
            const cJSON* lan_password = cJSON_GetObjectItemCaseSensitive(lan, "password");
            const cJSON* lan_token = cJSON_GetObjectItemCaseSensitive(lan, "discoveryToken");
            const cJSON* lan_clear_password = cJSON_GetObjectItemCaseSensitive(lan, "clearPassword");
            const cJSON* lan_clear_token = cJSON_GetObjectItemCaseSensitive(lan, "clearDiscoveryToken");
            const cJSON* remote_username = cJSON_GetObjectItemCaseSensitive(remote, "username");
            const cJSON* remote_host = cJSON_GetObjectItemCaseSensitive(remote, "host");
            const cJSON* remote_port = cJSON_GetObjectItemCaseSensitive(remote, "port");
            ...
            if (!cJSON_IsBool(enabled) || !cJSON_IsBool(remote_mode) ||
                ...
                (lan_token != nullptr && !cJSON_IsString(lan_token)) ||
                (lan_clear_password != nullptr && !cJSON_IsBool(lan_clear_password)) ||
                (lan_clear_token != nullptr && !cJSON_IsBool(lan_clear_token)) ||
                !cJSON_IsObject(remote) || !cJSON_IsString(remote_username) ||
                !cJSON_IsString(remote_host) || !cJSON_IsNumber(remote_port) ||
                (remote_password != nullptr && !cJSON_IsString(remote_password)) ||
                (remote_clear_password != nullptr && !cJSON_IsBool(remote_clear_password))) {
                cJSON_Delete(json);
                httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Invalid AIoT log configuration");
                return ESP_FAIL;
            }
            ...
            patch.lan.clear_password = cJSON_IsTrue(lan_clear_password);
            patch.lan.clear_discovery_token = cJSON_IsTrue(lan_clear_token);
            patch.lan.update_password = cJSON_IsString(lan_password) && lan_password->valuestring[0] != '\0';
            patch.lan.update_discovery_token = cJSON_IsString(lan_token) && lan_token->valuestring[0] != '\0';
            if (patch.lan.update_password) patch.lan.password = lan_password->valuestring;
            if (patch.lan.update_discovery_token) patch.lan.discovery_token = lan_token->valuestring;
            patch.remote.username = remote_username->valuestring;
            patch.remote.host = remote_host->valuestring;
            patch.remote.port = remote_port->valueint;
            patch.remote.clear_password = cJSON_IsTrue(remote_clear_password);
            patch.remote.update_password = cJSON_IsString(remote_password) && remote_password->valuestring[0] != '\0';
            if (patch.remote.update_password) patch.remote.password = remote_password->valuestring;

            std::string error;
            const bool saved = AiotLogConfigStore::Save(patch, &error);
            cJSON_Delete(json);
            httpd_resp_set_type(req, "application/json");
            httpd_resp_set_hdr(req, "Connection", "close");
            if (!saved) {
                const std::string reply = "{\"success\":false,\"error\":\"" + error + "\"}";
                httpd_resp_send(req, reply.c_str(), HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }
            // A restart makes the background client reread its configuration.
            // The HTTP response is fully sent first.
            httpd_resp_send(req, "{\"success\":true,\"restartRequired\":true}", HTTPD_RESP_USE_STRLEN);
            ESP_LOGI(TAG, "Saved optional local IoT log configuration (secrets hidden)");
            xTaskCreate([](void*) {
                vTaskDelay(pdMS_TO_TICKS(500));
                esp_restart();
            }, "aiot_log_restart", 2048, nullptr, 4, nullptr);
            return ESP_OK;
        },
        .user_ctx = this
    };
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &aiot_log_submit));
