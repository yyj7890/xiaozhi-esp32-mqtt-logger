...
## 19. 2026-08-02：启动、远程日志与 OLED 体验优化

### 已完成

- 配网页增加 AI 服务选择：官方服务模式跳过本地 AI 发现；本地服务模式才启用发现。该开关不改变官方 AI 配置。
- Wi-Fi 在首次成功连接后保存 AP 的 BSSID 和信道；后续启动优先直连，保存 AP 不可用时立即回退常规扫描。实测待命时间由约 6.8 秒缩短到约 3.4 秒。
- 远程日志 MQTT 保存最近一次由官方服务校准的可信 UTC 时间。后续启动在 Wi-Fi 就绪后立即并行建立 TLS MQTT，仍保留 CA bundle、域名校验、SNI 和证书有效期校验。实测待命后启动日志发布从约 5.4 秒缩短到约 0.2 秒。
- Bread Compact Wi-Fi 的 SSD1306 字幕滚动速度从 15 提升至 40；OLED I2C 从 100 kHz 提升到经折中的 200 kHz，未恢复已知不稳定的 400 kHz 设置。
- ESP-IDF v5.5.4 构建和 COM7 全量烧录均已通过，bootloader、应用、分区表、OTA 数据和资源分区均完成写入哈希校验；未擦除 NVS。

### 问题记录与归因纠正

- “待命后约 5 秒才看到 IoT 启动日志”已由串口时间线确认发生在设备端：远程 TLS MQTT 在可信时间取得后才开始连接，IoT 收到 MQTT 后即时入库。该问题不是 IoT 页面轮询导致。
- 页面出现多张“设备运行上报（N 条）”是 IoT 后端的启动会话归并问题：同一设备启动中的多个 MQTT 事件没有稳定归入同一记录。它与设备端单条 MQTT 发布无关，需在 `D:\AI\IOT` 后端单独修复，不能通过降低小智端上报频率掩盖。
- 上述归因不影响官方 AI 对话链路；远程日志持续采用低优先级、失败可重试的独立任务，不能阻塞待命、语音或官方服务激活。
