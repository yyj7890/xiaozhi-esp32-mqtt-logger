...
## 19. 室内环境待机屏（2026-08-05）

远程 AIoT MQTT 客户端新增可选的“待机屏室内传感器编号”配置，保存在同一私有 `aiot_log` NVS 命名空间。该编号只允许字母、数字、连字符和下划线，公开代码、文档和 Git 只使用配置占位，不包含实际编号。

配置后，小智在已建立的 LAN 或远程 TLS MQTT 连接上订阅：

```text
aiot/device/<environment-sensor-device-code>/report
```

收到合法 JSON 遥测后，客户端仅缓存 `temperature`、`humidity`、`pressure` 与 `illuminance`；MQTT 回调不直接操作显示硬件，而是通过 `Application::Schedule` 切换到主任务。设备处于 `Idle` 时显示室内温湿度、气压和光照；语音识别、连接、播放、配网、升级等状态始终优先。数据超过 3 分钟未更新时标记“室内数据已过期”。

该功能复用既有 MQTT TLS、CA 校验、SNI、凭据、QoS 1 和断线重连，不增加 HTTP 服务、不增加公网端口、不修改官方 AI、OTA、WebSocket、音频或主动播报 ACK 协议。环境订阅失败或无有效数据不会阻止小智启动和正常语音功能。

验证与发布边界：本次源码提交不包含私有 `sdkconfig`、NVS、环境设备编号、固件二进制或烧录操作。后续在目标板型私有配置下完成完整 ESP-IDF 构建、配网页保存、HiveMQ 收到遥测、待机显示、三分钟过期状态和语音/主动播报回归后，才可在用户明确授权下烧录。
