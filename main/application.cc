    ...
    board.SetNetworkEventCallback([this](NetworkEvent event, const std::string& data) {
        ...
        switch (event) {
            case NetworkEvent::Connecting:
                break;
            case NetworkEvent::Connected:
                MqttLogClient::GetInstance().RequestDiscovery();
                MqttLogClient::GetInstance().Report("NORMAL", "Wi-Fi connected");
                MqttLogClient::GetInstance().Log("wifi_connected", "INFO", "Wi-Fi connected");
                break;
            case NetworkEvent::Disconnected:
...
void Application::Run() {
    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_LOCAL_AI_FALLBACK) {
            HandleLocalAiFallbackEvent();
        }
...
void Application::HandleLocalAiFallbackEvent() {
    if (local_ai_fallback_started_.exchange(true)) {
        return;
    }
    // This handler runs on the main task, after a failed WebSocket operation
    // has returned. Releasing the old protocol here avoids racing its socket
    // callbacks from the network task.
    if (protocol_) {
    ...
    local_ai_route_selected_.store(false);
    if (local_ai_fallback_task_handle_ != nullptr) {
        return;
    }
    xTaskCreate([](void* arg) {
        Application* app = static_cast<Application*>(arg);
        app->OfficialFallbackTask();
        app->local_ai_fallback_task_handle_ = nullptr;
        vTaskDelete(nullptr);
    }, "official_fallback", 4096 * 2, this, 2, &local_ai_fallback_task_handle_);
}
...
void Application::ActivationTask() {
    ...
    if (!MqttLogClient::GetInstance().WaitForStartupVerification(12000, &mqtt_verification_reason)) {
        ...
        auto& board = Board::GetInstance();
        auto display = board.GetDisplay();
        display->SetChatMessage("system", "MQTT 账号密码错误或 Broker 不可用");
        Alert("本地日志 MQTT 验证失败", "MQTT 账号密码错误或 Broker 不可用。请连接 Xiaozhi-xxxx 配网页重新配置，或关闭本地日志。", "triangle_exclamation", Lang::Sounds::OGG_EXCLAMATION);
        // MQTT settings are entered through the captive portal. Re-open it
        // here so a bad username/password never leaves the user stranded in
        // idle state with no Xiaozhi-xxxx access point to correct it.
        SetDeviceState(kDeviceStateIdle);
        board.EnterWifiConfigMode();
        // EnterWifiConfigMode displays its own hotspot hint. Show the MQTT
        // failure again after that hint so the reason for reconfiguration is
        // visible on the device instead of being immediately overwritten.
        xTaskCreate([](void*) {
            vTaskDelay(pdMS_TO_TICKS(1500));
            auto& app = Application::GetInstance();
            if (app.GetDeviceState() == kDeviceStateWifiConfiguring) {
                app.Alert("MQTT 配置错误", "MQTT 账号或密码错误，请在配网页重新配置。", "triangle_exclamation", Lang::Sounds::OGG_EXCLAMATION);
            }
            vTaskDelete(nullptr);
        }, "mqtt_cfg_error", 3072, nullptr, 2, nullptr);
        return;
    }
...
void Application::OfficialFallbackTask() {
    // Do not reuse a local Ota instance or its endpoint. This re-check uses
    // the original official OTA URL and then creates a new official protocol.
    ota_ = std::make_unique<Ota>();
    CheckNewVersion();
    InitializeProtocol();
    ota_.reset();
...
void Application::CheckNewVersion() {
    ...
    while (true) {
        ...
        if (err != ESP_OK) {
            if (ota_->UsingTemporaryLocalEndpoint()) {
                ESP_LOGW(TAG, "Local AI OTA unavailable; falling back to official OTA");
                if (!local_ai_connection_failed_.exchange(true)) {
                    MqttLogClient::GetInstance().Log("local_ai_connection_failed", "ERROR", "Local AI OTA request failed");
                }
                local_ai_fallback_pending_.store(true);
                ota_->ClearTemporaryLocalEndpoint();
                continue;
            }
            retry_count++;
...
void Application::InitializeProtocol() {
    ...
    const bool local_websocket_route = ota_->UsingTemporaryLocalEndpoint() && ota_->HasWebsocketConfig();
    local_ai_route_selected_.store(local_websocket_route);

    if (ota_->HasMqttConfig()) {
        protocol_ = std::make_unique<MqttProtocol>();
    } else if (ota_->HasWebsocketConfig()) {
        protocol_ = std::make_unique<WebsocketProtocol>(ota_->GetTemporaryWebsocketUrl());
    } else {
    ...
    protocol_->OnConnected([this]() {
        if (local_ai_route_selected_.load()) {
            if (!local_ai_discovered_this_boot_.load()) {
                // Defensive: local connection notifications must always follow
                // a nonce-validated discovery result from this boot.
                return;
            }
            if (!local_ai_connection_failed_.load()) {
                static std::atomic_bool local_ai_connected_logged{false};
                if (!local_ai_connected_logged.exchange(true)) {
                    MqttLogClient::GetInstance().Log("local_ai_connected", "INFO", "Local AI WebSocket hello completed");
                }
            }
        } else {
            if (!official_ai_connected_.exchange(true)) {
    ...
    protocol_->OnNetworkError([this](const std::string& message) {
        MqttLogClient::GetInstance().Report("ABNORMAL", message);
        if (local_ai_route_selected_.exchange(false)) {
            if (!local_ai_connection_failed_.exchange(true)) {
                MqttLogClient::GetInstance().Log("local_ai_connection_failed", "ERROR", "Local AI WebSocket connection or hello failed");
            }
            local_ai_fallback_pending_.store(true);
            xEventGroupSetBits(event_group_, MAIN_EVENT_LOCAL_AI_FALLBACK);
        }
