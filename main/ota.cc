...
std::string Ota::GetCheckVersionUrl() {
    if (!temporary_ota_url_.empty()) return temporary_ota_url_;
...
void Ota::SetTemporaryLocalEndpoint(const std::string& ota_url, const std::string& websocket_url) {
    temporary_ota_url_ = ota_url;
    temporary_websocket_url_ = websocket_url;
}

void Ota::ClearTemporaryLocalEndpoint() {
    temporary_ota_url_.clear();
    temporary_websocket_url_.clear();
}
...
esp_err_t Ota::CheckVersion() {
    ...
    // A discovered local endpoint is deliberately session-only.  Its OTA
    // response may describe a websocket/mqtt route, but must never overwrite
    // the official route cached in NVS for later boots.
    const bool temporary_local_endpoint = UsingTemporaryLocalEndpoint();

    has_mqtt_config_ = false;
    cJSON *mqtt = cJSON_GetObjectItem(root, "mqtt");
    if (cJSON_IsObject(mqtt)) {
        if (!temporary_local_endpoint) {
