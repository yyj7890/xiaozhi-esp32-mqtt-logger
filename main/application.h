...
#define MAIN_EVENT_LOCAL_AI_FALLBACK    (1 << 13)
...
private:
    ...
    // These flags describe only the current boot.  They prevent reconnects,
    // cached settings and repeated error callbacks from producing misleading
    // duplicate local/official AI lifecycle events.
    std::atomic_bool local_ai_discovered_this_boot_{false};
    std::atomic_bool local_ai_route_selected_{false};
    std::atomic_bool local_ai_connection_failed_{false};
    std::atomic_bool official_ai_connected_{false};
